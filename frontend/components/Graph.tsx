...
interface GraphProps {
  onNodeClick: (pageNumber: number) => void;  // 추가된 prop
}
...
const Graph: React.FC<GraphProps> = ({ onNodeClick }) => {  // prop 추가
  ...
  useEffect(() => {
    ...
    const simulation = d3.forceSimulation<Node, Link>(data.nodes)
      .force("link", d3.forceLink<Node, Link>(data.links).id(d => (d as Node).id).distance(600))
    ...
    node.append("circle")
      ...
      .on("click", (event, d) => {
        ...
        const pageNumber = parseInt(id.replace('page_', '').replace('.txt', ''));
  ...
  return (
    <svg ref={svgRef} className='w-[600px] flex-shrink-0'></svg>
  );
};
